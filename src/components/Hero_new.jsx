...
const pulseMetrics = [
  { label: "Gaming Market", value: "$200B+", duration: 2.8, icon: TrendingUp, color: "from-emerald-500 to-cyan-500" },
  { label: "Launch Date", value: "April 25", duration: 2.2, icon: Zap, color: "from-purple-500 to-pink-500" },
  { label: "Early Access", value: "Limited", duration: 3.1, icon: Shield, color: "from-amber-500 to-orange-500" },
];
...
// Text animation variants for better word-by-word effect
      ...
      <div className="mx-auto grid w-full max-w-7xl items-center gap-10 px-4 sm:gap-12 sm:px-6 lg:grid-cols-2 lg:px-8">
        ...
        >
          ...
          <Motion.div
            ...
            className="space-y-2"
          >
            {Array.from({ length: Math.ceil(titleWords.length / 3) }).map((_, lineIndex) => (
              <div key={lineIndex} className="flex flex-wrap gap-2 sm:gap-3">
                {titleWords.slice(lineIndex * 3, (lineIndex + 1) * 3).map((word, idx) => (
                  <Motion.span
                    key={idx}
                    variants={wordVariants}
                    className={`text-4xl font-black leading-tight sm:text-5xl lg:text-6xl ${
                      idx === titleWords.length - 1 || idx === titleWords.length - 2
          ...
          {/* Enhanced Live Network Feed Card */}
          <Motion.div
            variants={animations.fadeInUp}
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className={`p-2 rounded-lg bg-gradient-to-br ${metric.color} bg-opacity-20`}>
                        <IconComponent size={16} className={metric.color.includes('emerald') ? 'text-emerald-400' : metric.color.includes('purple') ? 'text-purple-400' : 'text-amber-400'} />
                      </div>
                    </div>
